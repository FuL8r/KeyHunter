...
- **OSV-обогащение частичное.** Адаптер чинён: тянет `GET /v1/vulns/{id}`, пропускает
  404 и GIT-диапазоны (там хеши коммитов, не версии). Но OSV-запись по CVE-ID часто
  содержит только GIT-диапазоны, а версии по экосистемам — в связанных GHSA-записях,
  которые мы пока не разворачиваем. Поэтому `fixed_versions` из OSV нередко пуст.
  Это не критично: граница фикса есть в NVD (`version_end` = `versionEndExcluding`),
  и вердикт/сопоставление версий считаются по ней.
- *TODO:* в ответе использовать `affected.version_end` как «обновиться до …», когда
  `fixed_versions` пуст (сейчас в промпт идёт только `fixed_versions`).
- *TODO:* разворачивать OSV-алиасы (GHSA/PYSEC) ради версий по экосистемам.
